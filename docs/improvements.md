...
| 15 — mission feature half-disconnected       | Restored (not removed) — `TaskExecution` is mounted in `GameMasterView` again, `TaskCreation`/`TaskList` both have new test coverage. The separate `isGameActive` gating gap this item also flagged is unaddressed.  |
