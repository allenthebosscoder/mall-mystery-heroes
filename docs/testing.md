...
(4 tests), and `functions/scheduledFunctions/cleanupEndedRooms.integration.test.js`
(7 tests), 45 tests total.
...
| `joinRoom.integration.test.js`                                       | The `joinRoom` Cloud Function via `httpsCallable` (player access/room lifecycle): self-registration, duplicate name rejection, Lobby-phase gating                                    | 4     |
| `functions/scheduledFunctions/cleanupEndedRooms.integration.test.js` | The `cleanupEndedRooms` scheduled function via `firebase-functions-test` `wrap()`: room selection decision and actual deletion via the Admin SDK                                     | 7     |
