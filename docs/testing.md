...
`npm run test:emulator` runs eight further suites against the real Firestore,
...
`joinRoom.integration.test.js` (7 tests),
...
`submitKillPhoto.integration.test.js` (8 tests), and
`submitChatMessage.integration.test.js` (9 tests), 75 tests total.
...
| `test/firestore.rules.test.js`                                       | Security rules against the Firestore emulator, including the `allow list` grant that lets a host query their own rooms (item: dashboard removal), the narrow `allow create` grant letting a player write a `'chat'`-type, `recipient: null` `playerMessages` doc without general write access to the collection (chat-send-and-efficiency), the narrow `allow create` grant letting a player write a `photos` doc locked to `status: 'pending'`, `originalPlayerData: null` (kill-photo-submission), and both the allow and deny side of a joined player listing the full `players` collection (player-game-over-screen)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | 53    |
...
| `src/pages/PlayerGame.targetsIntegration.test.jsx`                   | Pins a final-review finding: renders the real `MessageComposer` + `KillPhotoModal` together, mounted with `targets={[]}` then re-rendered with `targets={['Bob']}` — matching `PlayerGame.js`'s actual pre-load/post-load sequence — and asserts a kill-photo submission targets `'Bob'`, not the stale empty initial value. The only test exercising this async-arrival rerender sequence — `MessageComposer.test.jsx` also renders the real `KillPhotoModal` now, but doesn't cover `targets` arriving after mount (kill-photo-submission)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | 1     |
