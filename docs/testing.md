...
Auth, and Functions emulators together — `dbCalls.integration.test.js` (31
...
(4 tests), 49 tests total.
`npm run test:rules` runs `test/firestore.rules.test.js` (48 tests) against
Firestore alone.
...
| `dbCalls.integration.test.js`                                        | The data layer against the Firestore emulator; includes test cases for `fetchPlayerMessagesQueryForRoom` and, for the chat-send-and-efficiency feature, `addChatMessageForRoom`'s written shape and the `limitToLast(50)` bound on `fetchPlayerMessagesQueryForRoom`                                                                                                                                  | 31    |
...
| `test/firestore.rules.test.js`                                       | Security rules against the Firestore emulator, including the `allow list` grant that lets a host query their own rooms (item: dashboard removal), and the narrow `allow create` grant letting a player write a `'chat'`-type, `recipient: null` `playerMessages` doc without general write access to the collection (chat-send-and-efficiency)                                                        | 48    |
...
| `src/pages/PlayerGame.test.jsx`                                      | Post-join player view: live status once `gameStarted` flips, shows target when alive or eliminated state when dead, redirect home if the room disappears, Leave clears the session, mounts `MessageFeed`/`MessageComposer` with the right props (item: join-flow UI and room scoping)                                                                                                                 | 12    |
...
| `MessageComposer.test.jsx`                                           | Player-facing message composer: the message input, Send button, and Enter-to-send all submit a typed message via `addChatMessageForRoom`, clearing the input afterward and rejecting blank/whitespace-only input; the photo button still renders disabled — kill-photo submission remains a separate, not-yet-built sub-project (chat-send-and-efficiency)                                            | 6     |
