...
| `MessageFeed.test.jsx`                                               | Player-facing live chat feed: subscription gating (no subscribe with an empty `playerName`), broadcast/whisper filtering (a whisper is visible only to its recipient, case/whitespace-insensitively), leaderboard messages rendered as a standings list, and auto-scroll to the newest message | 7     |
