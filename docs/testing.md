...
`npm run test:emulator` runs eleven further suites against the real Firestore,
...
`undoMissionCompletionCallable.integration.test.js` (10 tests), and
`removePlayerCallable.integration.test.js` (10 tests), 105 tests total.
`npm run test:rules` runs `test/firestore.rules.test.js` (52 tests) and
`test/storage.rules.test.js` (5 tests) against Firestore and Storage, 57
tests total.
