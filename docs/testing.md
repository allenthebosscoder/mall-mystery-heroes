...
| `KillPhotoModal.test.jsx`                                            | Kill-claim capture flow: auto-selects the only target and hides the picker when there's exactly one, shows a picker when there's more than one, calls `compressImage` → `uploadKillPhoto` → `addPhotoForRoom` in order then closes (an explicit `invocationCallOrder` assertion, not just call-argument checks), and keeps the modal open with an error (Submit still clickable) when the upload fails (kill-photo-submission)                                                                          | 4     |
| `src/pages/PlayerGame.targetsIntegration.test.jsx`                   | Pins a final-review finding: renders the real `MessageComposer` + `KillPhotoModal` together (neither stubbed, unlike every other test), mounted with `targets={[]}` then re-rendered with `targets={['Bob']}` — matching `PlayerGame.js`'s actual pre-load/post-load sequence — and asserts a kill-photo submission targets `'Bob'`, not the stale empty initial value (kill-photo-submission)                                                                                                          | 1     |
