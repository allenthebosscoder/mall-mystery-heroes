...
| `MessageFeed.test.jsx`                                               | Player-facing live chat feed: subscription gating (no subscribe with an empty `playerName`), broadcast/whisper filtering (a whisper is visible only to its recipient, case/whitespace-insensitively), leaderboard and mission-announcement messages rendered as formatted card content, and auto-scroll to the newest message | 7     |
| `MessageComposer.test.jsx`                                           | Player-facing message composer: the disabled message input, send button, and photo button all render disabled                                                                                                                                                                                                                 | 3     |
| `GameMasterView.test.jsx`                                            | Live header count and alive-only roster derivation (item 13); covers broadcasting kill/revive/open season/mission events to players' chat feed                                                                                                                                                                                | 13    |
