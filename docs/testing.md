...
`npm run test:rules` runs `test/firestore.rules.test.js` (51 tests) and
`test/storage.rules.test.js` (5 tests) against Firestore and Storage, 56
tests total.
...
| `test/firestore.rules.test.js`                                       | Security rules against the Firestore emulator, including the `allow list` grant that lets a host query their own rooms (item: dashboard removal), the narrow `allow create` grant letting a player write a `'chat'`-type, `recipient: null` `playerMessages` doc without general write access to the collection (chat-send-and-efficiency), and the narrow `allow create` grant letting a player write a `photos` doc locked to `status: 'pending'`, `originalPlayerData: null` (kill-photo-submission)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | 51    |
