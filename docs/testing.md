...
| `src/pages/PlayerGame.test.jsx`                                      | Post-join player view: live status once `gameStarted` flips, shows target when alive or eliminated state when dead, redirect home if the room disappears, Leave clears the session (item: join-flow UI and room scoping) | 10    |
