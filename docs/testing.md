...
`npm run test:emulator` runs six further suites against the real Firestore,
Auth, Storage, and Functions emulators together — `dbCalls.integration.test.js`
(29 tests), `executeKill.integration.test.js` (7 tests),
`joinRoom.integration.test.js` (7 tests),
`functions/scheduledFunctions/cleanupEndedRooms.integration.test.js`
(7 tests), `storageCalls.integration.test.js` (1 test), and
`undoKill.integration.test.js` (6 tests), 57 tests total.
`npm run test:rules` runs `test/firestore.rules.test.js` (53 tests) and
`test/storage.rules.test.js` (5 tests) against Firestore and Storage, 58
tests total.
...
| `dbCalls.integration.test.js`                                        | The data layer against the Firestore emulator; includes test cases for `fetchPlayerMessagesQueryForRoom` and, for the chat-send-and-efficiency feature, `addChatMessageForRoom`'s written shape and the `limitToLast(50)` bound on `fetchPlayerMessagesQueryForRoom`; and, for kill-photo-submission, `addPhotoForRoom` writing a pending photo with no `originalPlayerData`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | 29    |
...
| `PhotosDisplay.test.jsx`                                             | Reload-recovery for photo undo (item 6); validation and Cloud Function response routing (items 4, 5); undo now delegates to the atomic `undoKillPlayer` Cloud Function via `undoKill` (2026-08-16 full-kill-undo redesign)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | 6     |
...
| `MessageFeed.test.jsx`                                               | Player-facing live chat feed: subscription gating (no subscribe with an empty `playerName`), broadcast/whisper filtering (a whisper is visible only to its recipient, case/whitespace-insensitively), leaderboard and mission-announcement messages rendered as formatted card content, auto-scroll to the newest message, and (chat-send-and-efficiency) a `'chat'` message rendered with its sender                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | 10    |
| `MessageComposer.test.jsx`                                           | Player-facing message composer: the message input, Send button, and Enter-to-send all submit a typed message via `addChatMessageForRoom`, clearing the input afterward and rejecting blank/whitespace-only input (chat-send-and-efficiency); the photo button is enabled only when both `playerName` and a non-empty `targets` are set (kill-photo-submission). Since the 2026-08-15 one-tap-kill-photo-capture redesign, this file owns the capture flow itself — `KillPhotoModal` is rendered for real, not stubbed — asserting the camera button's `onClick` calls the hidden file input's `click()`, running the full compress → preview → Submit → upload → `addPhotoForRoom` sequence in order, handling a compression failure and an upload failure separately, resetting the input's `value` after each selection, showing a processing indicator immediately on capture and before compression resolves, and confirming the hidden input's own `disabled` guard — not just the button's — blocks a file selection when `playerName` or `targets` is empty, revoking the preview URL on unmount, and revoking the previous preview URL when a second capture fails to compress | 22    |
| `GMChatPanel.test.jsx`                                               | Read-only GM-console view of the players' group chat, a separate panel from the GM's own game-event Logs: renders a `'chat'` message with its sender, filters out non-chat messages (e.g. a GM broadcast), gates the subscription on a non-empty `roomID`, and auto-scrolls to the newest message (chat-send-and-efficiency)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | 4     |
...
redesign), `functions/callableFunctions/joinRoom.js`, and
...
for reads, host-only for writes, with one narrow exception each: `photos`
now also grants a player a `create` locked to `status: 'pending'` and
`originalPlayerData: null` (kill-photo-submission — a player can submit a
kill claim but cannot self-approve it or forge the pre-kill snapshot Undo
relies on), and `playerMessages` grants a player a `create` locked to
`type: 'chat'`, `recipient: null` (chat-send-and-efficiency, above). Neither
is scoped to a distinct per-uploader/per-sender identity — this app has no
such identity to check against, only "some player of this room."
