...
const ResetTargetsButton = ({ arrayOfPlayers, addLog }) => {
    ...
    const onYesClose = async () => {
        ...
        for (const player of arrayOfPlayers) {
            await addLog(
                'New Target(s) for ' + player + ': ' + targetMap.get(player).join(', '),
                'blue.400'
            );
    ...
    const InitializeTargets = () => {
        ...
        for (let playerDex = 0; playerDex < arrayOfPlayers.length; playerDex++) {
            ...
            for (let targetCount = 0; targetCount < MAXTARGETS; targetCount++) {
                let targetIndex =
                    (newPlayerData[currPlayer].lastTargetIndex + 1) % arrayOfPlayers.length;
                ...
                while (
                    newPlayerData[target].numOfAssassins === MAXTARGETS ||
                    target === currPlayer ||
